...
On Linux systems like the Debian one I use, there are documentation pages available already installed. There are hundreds of them and yet only 6 are required to create all of my command line tools.


The following commands can be used to read each one of the 6 fundamental system calls available on Linux and Unix systems to C programmers.
...
But after reading about the system calls and seeing some example, I realized that I could make copies and rewrite these tools by calling only system calls. During this process I learned how much easier they are to use compared to the C library functions.

Here is a brief summary of each of these functions and how they are used in my programs.

All of my tools use "open" to open a file and then "close" to close them when I am done with them. There can be no confusion as to what these functions do because of their names.

Similarly, "read" and "write" do exactly what their names imply. They operate the same as fread and fwrite do in stdio.h but they take only 3 arguments instead of 4, which makes a lot of sense. You give them a pointer and then you tell them exactly how many bytes you want to read from or write to a file descriptor previously assigned with "open".
...
Each of these functions has various arguments which have clearly defined meanings. The return values are also specified in their manual pages. 
