...
int next_file_input()
{
 ...
 else
 {
  ...
  /*
   replace color from SDL version with character id for this block
   This is necessary because we are using text, not colored SDL squares.
  */
  main_block.color=main_block.id;
...
/*these are used for debugging but not the rest of the time*/
  ...
  */
  ...
  if(key==' '){key='h';}
  if(key=='h'){move_id='H';strcpy(text,"Hard Drop");tetris_hard_drop();}
  
  if(key=='q'){key=0x1B;} /*if q then set to escape as another way of ending the program. */
...
/*
 This is C program using the ncurses library. It can be compiled and run like this.

 gcc -Wall -ansi -pedantic main.c -o main -lncurses && ./main

 This program is an ncurses port of my Chaste Tris game originally written using SDL and published on Steam.
 It serves as a functional demo on platforms that do not allow installing SDL.
*/
